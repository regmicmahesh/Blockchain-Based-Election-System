...
async function initContract(jsonfile, accountAddr) {
  ...
  contract = await contract
    .deploy({ data: contractBC })
    .send({ from: accountAddr, gas: 1500000, gasPrice: "30" });
  return contract;
}
...
async function distributeBalance(contractManager, value, unit = "ether") {
  ...
  await contractManager.contract.methods
    .Distribute(value.toString())
    .send({ from: contractManager.accountAddr });
}
